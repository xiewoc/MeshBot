...
class AsyncDeepSeekChatClient:
    def __init__(self, api_key: str, base_url: str = "https://api.deepseek.com", 
                 default_model: str = "deepseek-chat", organization: Optional[str] = None):
    ...
    async def init(self):
        ...
        if self.session is None or self.session.closed:
            ...
            self.logger.info(f"DeepSeek客户端已初始化，默认模型: {self.default_model}")
    ...
    async def chat(self, user_name, message: str, model: Optional[str] = None, 
        ...
        async with self._lock:
            try:
                ...
                payload: Dict[str, Any] = {
                    "model": model,
                    "messages": messages,
                    "temperature": max(0.1, min(2.0, temperature)),  # DeepSeek支持0-2范围:cite[1]
                    "stream": stream
                }
                ...
                async with self.session.post(f"{self.base_url}/chat/completions", 
                    ...
                    if resp.status == 200:
                        ...
                        self.logger.error(f"DeepSeek API错误: {resp.status} - {error_data}")
                        return {
                            "success": False, 
    ...
    def _update_conversation_history(self, user_message: str, ai_response: str):
        ...
        # 限制历史记录长度（DeepSeek支持128K上下文:cite[4]）
